...
prompt.get(['name'],function (err,result) {
	...
	mkdirp('./' + '@' + mainfoldername + '/',function(err) {
		...
		prompt.get(['description'],function(err,result) {
			...
			modcppicon = 'logo = ' + '"' + 'w_guitar_ca.paa' + '"' + ';';
			modcpppicture = 'picture = ' + '"' + 'w_guitar_ca.paa' + '"' + ";";
							...
							fs.readdir(songfolder, function(err,files) {
								...
								function generateconfigcpp() {
									if (filesinfoprocessed == filesinfo2process && configcppgenerationfinished == 0){
										...
										var cfgvehiclesstart = os.EOL + 'class CfgVehicles {' + os.EOL + '\tclass Man;' + os.EOL + '\tclass CAManBase: Man {' + os.EOL + '\t' + '\t' + 'class ACE_SelfActions {' + os.EOL + '\t' + '\t' + '\t' + 'class guitarActions' + os.EOL + '\t' + '\t' + '\t' + '{' + os.EOL + '\t' + '\t' + '\t' + '\t' + 'class guitarSongs' + os.EOL + '\t' + '\t' + '\t' +'\t' + '{'
										...
										function createnewplaysongcategory() {
											...
											cfgvehiclesmiddlepart += os.EOL + '\t' + '\t' + '\t' + '\t' + 'class ' + 'guitarSongs' + createdsubcategories;
											cfgvehiclesmiddlepart += os.EOL + '\t' + '\t' + '\t' + '\t' + '{';
										...
										for (i = 0; i < songs.length; i++) {
											...
											cfgvehiclesmiddlepart += '\t' + '\t' + '\t' + '\t' + '\t' + '\tstatement = "terminate guitar_script; guitar_script = [' + "'" + i_[0].replace(/-|&|@|\[|\]|\#|\%|\*|\^|\!|\'|\"|\.|,| /gi, "_") + "'" + ',' + Math.round(Number(songslenghts[i]) + 1) + ',' + '200] execVM ' + "'" + '\\ussr_guitar\\scripts\\playguitar.sqf' + "'" + '";' + os.EOL
